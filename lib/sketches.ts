...
export async function getAllSketches(): Promise<Sketch[]> {
  ...
  const sketches = filenames.map(async (filename: string) => getSketchByFilename(filename))
  return await Promise.all(sketches).then((sketches) => sketches.sort((sketch1, sketch2) => (sketch1.date > sketch2.date ? -1: 1)))
}
